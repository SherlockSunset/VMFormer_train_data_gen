...
def add_green_light(image_path, pha_image_path, green_img, random_float):
    # Load the image
...
if __name__ == '__main__':
    ...
    current_fgr_data_dir_overall = os.path.join(current_data_dir, 'fgr')

    all_video_subfolders = os.listdir(current_fgr_data_dir_overall)

    print('Videos num: ', len(all_video_subfolders))
    ...
    for current_video in tqdm(all_video_subfolders):
        ...
        print('Random random_float: ', random_float)
        print('Random random_index: ', random_index)

        current_fgr_data_dir = os.path.join(current_fgr_data_dir_overall, current_video)
        ...
        current_pha_data_dir = current_fgr_data_dir.replace('fgr', 'pha')
        all_pha_images = glob(os.path.join(current_pha_data_dir, '*' + img_format))
        all_pha_images = sorted(all_pha_images)
        
        
        dst_fgr_save_dir = current_fgr_data_dir.replace(current_data_dir, dst_save_dir)
        ...
        for image_path, pha_image_path in zip(all_images, all_pha_images):
            ...
            result, mask = add_green_light(image_path, pha_image_path, green_img, random_float)
        ...
        dst_pha_save_dir = current_fgr_data_dir.replace(current_data_dir, dst_save_dir).replace('fgr', 'pha')
